...
1. **"Dynamic" = three layers, each overridable.**
   ...
   - *Master discretion* (runtime) — right-size, fan out, insert a review, escalate — **bounded by `gate: standard|critical` steps** that can never be skipped. This is what makes it adaptive, not a rigid DAG and not a free-for-all.
